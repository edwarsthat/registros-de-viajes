 ...
 * 
 * Formulario pre-cargado con los datos del viaje
...
try {
    $stmt = $pdo->prepare("SELECT * FROM viaje WHERE idviaje = ?");
    $stmt->execute([$id]);
    $viaje = $stmt->fetch();
    
    if (!$viaje) {
        header('Location: listar.php?error=Viaje no encontrado');
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        try {
            // Actualizar viaje en la base de datos
            $stmt = $pdo->prepare("UPDATE viaje SET idcarro = ?, idciudad_origen = ?, idciudad_destino = ?, tiempo_horas = ? WHERE idviaje = ?");
            $stmt->execute([$idcarro, $idciudad_origen, $idciudad_destino, $tiempo_horas, $id]);
            
            $exito = true;
            
            // Actualizar datos del viaje para mostrar en el formulario
            $viaje['idcarro'] = $idcarro;
            $viaje['idciudad_origen'] = $idciudad_origen;
            $viaje['idciudad_destino'] = $idciudad_destino;
            $viaje['tiempo_horas'] = $tiempo_horas;
            
            // Redirigir al listado después de 2 segundos
            header('Refresh: 2; URL=listar.php?msg=Viaje actualizado exitosamente');
        } catch (PDOException $e) {
            $errores[] = 'Error al actualizar el viaje: ' . $e->getMessage();
        }
    }
}
                ...
                <div class="card-body">
                    ...
                    <?php if ($exito): ?>
                        <div class="alert alert-success" role="alert">
                            <strong>¡Éxito!</strong> El viaje ha sido actualizado correctamente.
                    ...
                    <form method="POST" action="" class="needs-validation" novalidate>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="idcarro" name="idcarro" required>
                                ...
                                <option value="<?= $carro['idcarro'] ?>" <?= ($viaje['idcarro'] == $carro['idcarro']) ? 'selected' : '' ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="idciudad_origen" name="idciudad_origen" required>
                                ...
                                <option value="<?= $ciudad['idciudad'] ?>" <?= ($viaje['idciudad_origen'] == $ciudad['idciudad']) ? 'selected' : '' ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="idciudad_destino" name="idciudad_destino" required>
                                ...
                                <?php foreach ($ciudades as $ciudad): ?>
                                <option value="<?= $ciudad['idciudad'] ?>" <?= ($viaje['idciudad_destino'] == $ciudad['idciudad']) ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($ciudad['nombre']) ?>
                                </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input 
                                ...
                                id="tiempo_horas" 
                                name="tiempo_horas" 
                                value="<?= htmlspecialchars($viaje['tiempo_horas']) ?>"
                                required
                                min="1"
                                step="1"
                                placeholder="Ej: 8"
                        ...
                        <div class="mb-3">
                            <div class="alert alert-info">
                                <strong>Fecha de registro:</strong> <?= date('d/m/Y H:i', strtotime($viaje['fecha'])) ?>
                            </div>
                        </div>
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-save"></i> Actualizar Viaje
                            </button>
